...
class Node(object):
    __slots__ = ('value', 'prev', 'next')   # save memory  #Python允许在定义class的时候，定义一个特殊的__slots__变量，来限制该class能添加的属性。例如：当定义一个class 后，想要限制它的属性时，我们可以用 __slots__ 添加 age 、name、color 属性，再调用。
    #https://blog.csdn.net/deweicengyou/article/details/88855485
